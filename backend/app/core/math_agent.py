...
class AgentState(BaseModel):
    question: str
    retrieved_content: str = Field(default="")
    solution: str = Field(default="")
    source: Literal["knowledge_base", "web_search", "error"] = Field(default="error")

@tool    
def agent_router(state: AgentState) -> Literal["kb_search", "web_search"]:
    ...
    if len(state.retrieved_content) > 500:  # Arbitrary length threshold
...
workflow.add_conditional_edges(
    "router",
    lambda state: "kb_solution_generator" if state.retrieved_content else "web_solution_generator",
